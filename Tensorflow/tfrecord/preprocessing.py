...
def crop_center(image):
    ...
    image = tf.image.crop_to_bounding_box(image, offset_y, offset_x, new_shape,
                                          new_shape)
    return image
...
def pad(image):
    ...
    image = tf.image.pad_to_bounding_box(image, offset_y, offset_x, new_shape,
                                         new_shape)
    return image
...
def pad_200(image):
    ...
    shape = tf.shape(image)
    image = tf.image.pad_to_bounding_box(image, 0, 200, shape[0],
                                         shape[1] + 400)
    shape = tf.shape(image)
    new_shape = tf.minimum(shape[0], shape[1])
    offset_y = tf.maximum(shape[0] - shape[1], 0) // 2
...
def crop_or_pad(image, curr_height, curr_width, new, height=True, crop=True):
  ...
  """
    ...
    abs_diff = tf.abs(new - curr_height) // 2 if height else tf.abs(
        new - curr_width) // 2
    ...
    if crop:
        ...
        image = tf.image.pad_to_bounding_box(image, offset_y, offset_x,
                                             target_height, target_width)
    return image
...
def decode_image(image_str):
    ...
    bgr = tf.concat([blue, green, red], 2)
    return bgr
...
def preprocess_training_images(images,
  ...
  Args:
    images: A 4-D float32 `Tensor` holding raw images to be preprocessed.
            or a 3-D float32 Tensor without batch
...
def preprocess_training_image(image,
  ...
  """
    ...
    if flip_left_right:
        do_a_flip_lr_random = tf.greater(tf.random_uniform([], seed=seed), 0.5)
        image = tf.cond(do_a_flip_lr_random,
                        lambda: tf.image.flip_left_right(image), lambda: image)
    if flip_up_down:
        do_a_flip_ud_random = tf.greater(tf.random_uniform([], seed=seed), 0.5)
        image = tf.cond(do_a_flip_ud_random,
                        lambda: tf.image.flip_up_down(image), lambda: image)

    if rotate_90:
        do_a_rotate = tf.greater(tf.random_uniform([], seed=seed), 0.5)
        image = tf.cond(do_a_rotate, lambda: tf.image.rot90(image),
                        lambda: image)
...
def preprocess_images(images,
  ...
  """
    if is_training:
        return preprocess_training_images(images, height, width, min_scale,
                                          max_scale, p_scale_up, aug_color,
                                          fast_mode)
    else:
...
def preprocess_image(image,
  ...
  """
    if is_training:
        return preprocess_training_images(image, height, width, min_scale,
                                          max_scale, p_scale_up, aug_color,
                                          fast_mode)
    else:
        return preprocess_test_images(image, height, width, crop_strategy)
